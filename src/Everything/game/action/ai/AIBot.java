...
public class AIBot {
    ...
    private WeJustDidThisMove doATilePlacementMove(final Map map, final Player player, final Tile tile) {
        System.out.println("BOT: Doing a tile placement");
    ...
    private WeJustDidThisMove expandIfResultingSettlementIsSizeX(Map map, Settlement settlement, Player player, int sizeX) throws NoValidActionException, CannotPerformActionException {
        ...
        for (Terrain currentTerrain : Terrain.values()) {
            if (currentTerrain != Terrain.VOLCANO) {
                ...
                if (settlement.size() + expansionSpots.size() == sizeX) {
                    settlementExpansionHandler.expandWithMeeples(expansionSpots, map, player);
    ...
    private WeJustDidThisMove foundMeepleToIncreaseSettlementSize(Map map, Settlement settlement, Player player) throws NoValidActionException, CannotPerformActionException {
        ArrayList<MapSpot> foundingSpots = foundingNextToSettlementScanner.scan(settlement, map);
        return settlementFoundingHandler.foundSettlement(foundingSpots.get(0), map, player);
    }
...
//                        System.out.println("friendlySettlement.size() >= 5 && friendlySettlement.getNumberOfTotoros() < 1");
                        try {
                            ...
                            return settlementExpansionHandler.expandWithTotoro(totoroSpots.get(0), map, player);
...
//                        System.out.println("friendlySettlement.size() == 2 && friendlySettlement.getNumberOfTotoros() < 1");
                        ...
                        try {
                            return expandIfResultingSettlementIsSizeX(map, friendlySettlement, player, 4);
                        } catch (NoValidActionException | CannotPerformActionException e) {
                        }
...
//                        System.out.println("friendlySettlement.size() == 1 && friendlySettlement.getNumberOfTotoros() < 1");
                        ...
                        try {
                            return expandIfResultingSettlementIsSizeX(map, friendlySettlement, player, 3);
                        } catch (NoValidActionException | CannotPerformActionException e) {
                        }
                        ...
                        try {
                            ...
                            return settlementFoundingHandler.foundSettlement(foundingSpots, map, player);
                        } catch (NoValidActionException | CannotPerformActionException e) {
                            e.printStackTrace();
                        }
                    }
                        ...
                        if (expansionTerrain != Terrain.VOLCANO) {
                            ...
                            if (player.isHasEnoughMeeples(meeplesCost)) {
                                ...
                                if (meeplesCost == player.getNumberOfMeeplesLeft()) {
                                    try {
                                        settlementExpansionHandler.expandWithMeeples(expandableSpots, map, player);
                                        WeJustDidThisMove expansionMove = new WeJustDidThisMove();
                                        expansionMove.setBuildType(2);
            ...
            if (maxCost > 1 && maxExpansionSpots != null && maxSettlementToExpand != null && maxTerrain != null &&
                ...
                try {
                    settlementExpansionHandler.expandWithMeeples(maxExpansionSpots, map, player);
                    WeJustDidThisMove expansionMove = new WeJustDidThisMove();
                    expansionMove.setBuildType(2);
