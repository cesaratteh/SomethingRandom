...
public class Updater {
    ...
    public void EnemyMove(Map map, int MoveNumber, Terrain A, Terrain B, MapSpot TileSpot, int Orientation, int TurnChoice, MapSpot ExpandLocation){
        ...
        if(map.getHexagon(TileSpot) == null){

            TileLevel = 1;
        }
        else{
            TileLevel = map.getHexagon(TileSpot).getLevel() +1;
        }
        ...
        map.setHexagon(TileSpot, hex1);
        Hexagon hex2 = new Hexagon(A,TileLevel,MoveNumber);
        map.setHexagon(hexSpot2, hex2);
        Hexagon hex3 = new Hexagon(B,TileLevel,MoveNumber);
        map.setHexagon(hexSpot3, hex3);




        switch(TurnChoice){
            case 1: map.getHexagon(ExpandLocation).addMeeplesAccordingToLevel(Team);
            break;
            case 3: map.getHexagon(ExpandLocation).addTotoro(Team);
            break;
            case 4: map.getHexagon(ExpandLocation).addTiger(Team);
            break;
        }
    }
...
//            if(ListOfSettlements.get(i).isMapSpotInSettlement(ExpandLocation) == true){
//                EnemySettlemenst = ListOfSettlements.get(i);
//
//            }
//        }
//
