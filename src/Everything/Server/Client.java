...
//                for (ConcurrentLinkedQueue<MoveData> threadsQueue : runningThreadsMap.values()) {
                for(String key: runningThreadsMap.keySet()){

                    synchronized (runningThreadsMap.get(key)) {
                        if (!runningThreadsMap.get(key).isEmpty()) {
                            if (runningThreadsMap.get(key).peek().consumer == MoveData.Consumer.CLIENT) {

                                MoveData moveData = runningThreadsMap.get(key).poll();

                                if (moveData.move instanceof WeJustDidThisMove) {

                                    String friendlyMoveMessageToBeSent = tip.createFriendlyMoveMessageToBeSent((WeJustDidThisMove) moveData.move, key);
