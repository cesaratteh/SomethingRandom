...
public class AIRivalBot {
    ...
    public void doABuildMove(final Map map) {
        ...
        if (player.getNumberOfTotorosLeft() > 0) {
            if (Settlement.size() = 0) {
                ...
                for (Settlement friendlySettlement : friendlySettlements) {
                    if (friendlySettlement.size() >= 5 && friendlySettlement.getNumberOfTotoros() < 1) {
                        ...
                        // Place villager on any hex of tile being placed
                        return;
                    }
                }
            ...
            //  c. stop when difference = 0
            // If all costs for expansion are greater than meeple count then found villager in any random spot
            return;
        }
        else {
            return; // Game Over
